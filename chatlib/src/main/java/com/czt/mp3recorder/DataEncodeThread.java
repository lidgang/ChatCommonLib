...
public class DataEncodeThread extends Thread implements AudioRecord.OnRecordPositionUpdateListener {
    ...
    private CountDownLatch handlerInitLatch = new CountDownLatch(1);//线程同步的方法
    ...
    public Handler getHandler() {
        try {
            handlerInitLatch.await(); //调用该方法的时候一直会堵塞该线程
    ...
    private int processData() {
        ...
        if (bytes > 0) {
            ...
            int encodedSize = LameUtil.encode(innerBuf, innerBuf, bytes / 2, mp3Buffer);
